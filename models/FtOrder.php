 ...
 */
class FtOrder extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public function getSurplusGruop()
    {
        ...
        $order_detail = CmOrderDetail::findOne(['order_id' => $order_id, 'is_delete' => 0]);
        if ($this->class_group) {
            $goods = CmGoodsDetail::findOne(['id' => $this->class_group]);
        } else {
            $goods = CmGoods::findOne(['id' => $order_detail->goods_id]);
        }
        $groupNum = CmOrder::find()
    ...
    {
        return $this->hasMany(CmOrderDetail::className(), ['order_id'=>'id'])->alias('od')
            ->leftJoin(['g'=>CmGoods::tableName()], 'g.id=goods_id')->select(['od.*','g.name','g.attr goods_attr']);
    }

    // 获取指定商品指定用户已购买次数
    public static function getCount($goods_id,$user_id)
    {
        $query = CmOrderDetail::find()->where(['is_delete'=>0,'goods_id'=>$goods_id]);
