...
class Goods extends \yii\db\ActiveRecord
{
    ...
    //获取一个分类
    public static function getCatOne($goods)
    {
        $cat_list = [];
//        var_dump($goods->cat_id);exit;
        $cat_list = Cat::find()->where(['id'=>$goods->cat_id,'store_id'=>$goods->store_id])->asArray()->all();
        if(!$cat_list){
            $cat_list[0]['cat_id'] = "";
            $cat_list[0]['name'] = "";
        }
    ...
    public function getGoodCat(){
        $cat=Cat::findOne(['id'=>$this->cat_id,'store_id'=>$this->store_id,'is_delete'=>0]);
        return $cat->name;
    }
