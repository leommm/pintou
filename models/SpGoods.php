 ...
 */
class SpGoods extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public function rules()
    {
        return [
            ...
            [['store_id', 'cat_id', 'status', 'grouptime', 'sort', 'virtual_sales', 'weight', 'freight', 'addtime', 'is_delete', 'group_num', 'is_hot', 'limit_time', 'is_only', 'is_more', 'buy_limit', 'type', 'use_attr', 'one_buy_limit', 'mch_id'], 'integer'],
    ...
    {
        return SpGoodsPic::findAll(['goods_id'=>$this->id,'is_delete'=>0]);
    }
 ...
 */
    ...
    public function getAttrGroupListnum()
    {
        ...
        $goodsdetail = SpGoodsDetail::find()->where(['store_id'=>$this->store_id])->andWhere('goods_id=:goods_id', [':goods_id'=>$this->id])->all();
        $goods = (object)null;
    ...
    public function getAttrInfo($attr_id_list, $id = null)
    {
        ...
        if ($id) {
            $list = SpGoodsDetail::find()->where(['store_id'=>$this->store_id])->andWhere('id=:id', [':id'=>$id])->one();
    ...
    {
        $res = SpOrderDetail::find()->alias('od')
            ->select('SUM(od.num) AS sales_volume')
            ->leftJoin(['o' => SpOrder::tableName()], 'od.order_id=o.id')
    ...
    {
        $goods = SpGoods::findOne($goods_id);
        if (!$goods) {
            return null;
        }
