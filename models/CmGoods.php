 ...
 */
class CmGoods extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public function rules()
    {
        return [
            ...
            [['store_id','date_start','date_end','activity_type','cat_id', 'status', 'grouptime', 'sort', 'virtual_sales', 'weight', 'freight', 'addtime', 'is_delete', 'group_num', 'is_hot', 'limit_time', 'is_only', 'is_more', 'buy_limit', 'type', 'use_attr', 'one_buy_limit', 'mch_id'], 'integer'],
            [['original_price', 'price', 'colonel'], 'number'],
            [['detail', 'attr', 'cover_pic','live_link'], 'string'],
    ...
    {
        return CmGoodsPic::findAll(['goods_id'=>$this->id,'is_delete'=>0]);
    }
 ...
 */
    ...
    public function getAttrGroupListnum()
    {
        ...
        $goodsdetail = CmGoodsDetail::find()->where(['store_id'=>$this->store_id])->andWhere('goods_id=:goods_id', [':goods_id'=>$this->id])->all();
        $goods = (object)null;
    ...
    {
        $res = CmOrderDetail::find()->alias('od')
    ...
    {
        $goods = CmGoods::findOne($goods_id);
        if (!$goods) {
            return null;
        }
