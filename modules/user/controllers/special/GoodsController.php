...
namespace app\modules\user\controllers\special;
...
class GoodsController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $cat_list = SpCat::find()->select('id,name')->andWhere(['store_id' => $this->store->id, 'is_delete' => 0, 'mch_id' => $this->mch->id])->orderBy('sort ASC')->asArray()->all();
    ...
    {
        $cat = SpCat::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id, 'mch_id' => $this->mch->id]);
        if (!$cat) {
            $cat = new SpCat();
        }
    ...
    {
        $goods = SpGoods::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id, 'mch_id' => $this->mch->id]);
