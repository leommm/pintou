...
namespace app\modules\user\controllers\factory;
...
use app\modules\mch\models\group\FtCatForm;
use app\modules\mch\models\group\FtGoodsDetailForm;
...
class GoodsController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $arr = $form->getList($this->store->id,$this->mch->id);
        ...
        $cat_list = FtCat::find()->select('id,name')->andWhere(['store_id' => $this->store->id, 'is_delete' => 0, 'mch_id' => $this->mch->id])->orderBy('sort ASC')->asArray()->all();
    ...
    {
        $cat = FtCat::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id, 'mch_id' => $this->mch->id]);
        if (!$cat) {
            $cat = new FtCat();
        }
    ...
    {
        $cat = FtCat::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id]);
        if (!$cat) {
            return [
                'code' => 1,
                'msg' => '分类不存在或已删除',
    ...
    {
        $query = FtGoodsDetail::find()->alias('od')->select(['od.*', 'g.name', 'c.name AS cname', 'g.status'])
            ->where(['od.store_id' => $this->store->id, 'g.is_delete' => 0, 'g.store_id' => $this->store->id])
            ->leftJoin(['g' => FtGoods::tableName()], 'g.id=od.goods_id')
            ->leftJoin(['c' => FtCat::tableName()], 'g.cat_id=c.id');
    ...
    {
        $model = FtGoodsDetail::findOne(['id' => $id, 'store_id' => $this->store->id]);
        if (\Yii::$app->request->isPost) {
            $form = new FtGoodsDetailForm();
            $form->store_id = $this->store->id;
            $form->attributes = \Yii::$app->request->post();
            $form->attr = \Yii::$app->serializer->encode(\Yii::$app->request->post('attr'));

            if (!$model) {
                $model = new FtGoodsDetail();
        ...
        if ($goods_id) {
            $goods = FtGoods::find()->where(['is_delete' => 0, 'store_id' => $this->store->id])->andWhere('id=:id', [':id' => $goods_id])->one();
            $goods_attr = json_decode($goods['attr'], true);
        }
        if ($id) {
            $goods_detail = FtGoodsDetail::find()->alias('od')->select(['od.*', 'g.attr AS gattr', 'g.name'])
    ...
    {
        $goods = FtGoods::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id, 'mch_id' => $this->mch->id]);
        ...
        $ptCat = FtCat::find()
            ->andWhere(['is_delete' => 0, 'store_id' => $this->store->id, 'mch_id' => $this->mch->id])
            ->asArray()
            ->orderBy('sort ASC')
            ->all();
    ...
    {
        if ($type == 'down') {
            $goods = FtGoods::findOne(['id' => $id, 'is_delete' => 0, 'status' => 1, 'store_id' => $this->store->id]);
            if (!$goods) {
                return [
                    'code' => 1,
                    'msg' => '商品已删除或已下架',
            ...
            $goods = FtGoods::findOne(['id' => $id, 'is_delete' => 0, 'status' => 2, 'store_id' => $this->store->id]);
            if (!$goods) {
                return [
                    'code' => 1,
                    'msg' => '商品已删除或已上架',
            ...
            $goods = FtGoods::findOne(['id' => $id, 'is_delete' => 0, 'is_hot' => 0, 'store_id' => $this->store->id]);
            if (!$goods) {
                return [
                    'code' => 1,
                    'msg' => '商品已删除或已设为热销',
            ...
            $goods = FtGoods::findOne(['id' => $id, 'is_delete' => 0, 'is_hot' => 1, 'store_id' => $this->store->id]);
            if (!$goods) {
                return [
                    'code' => 1,
                    'msg' => '商品已删除或已取消热销',
    ...
    public function actionBatch()
    {
        ...
        if ($get['type'] == 0) { //批量上架
            $res = FtGoods::updateAll(['status' => 1], $condition);
        } elseif ($get['type'] == 1) { //批量下架
            $res = FtGoods::updateAll(['status' => 2], $condition);
        } elseif ($get['type'] == 2) { //批量删除
            $res = FtGoods::updateAll(['is_delete' => 1], $condition);
        } elseif ($get['type'] == 3) { //批量设置热销
            $res = FtGoods::updateAll(['is_hot' => 1], $condition);
        } elseif ($get['type'] == 4) { //批量取消热销
            $res = FtGoods::updateAll(['is_hot' => 0], $condition);
        }
        if ($res > 0) {
            return [
                'code' => 0,
    ...
    {
        $goods = FtGoods::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id]);
        if (!$goods) {
            return [
                'code' => 1,
                'msg' => '商品删除失败或已删除',
