	...
	<div class="panel-body">
		...
		<div class="mb-3 clearfix">
			<div class="float-left">
				<a href="<?= $urlManager->createUrl(['user/bargain/goods/goods-edit']) ?>" class="btn btn-primary"><i
				...
				<div class="dropdown float-right ml-2">
					...
					<div class="dropdown-menu" aria-labelledby="dropdownMenuButton"
						...
						<a href="javascript:void(0)" class="btn btn-secondary batch dropdown-item"
						   data-url="<?= $urlManager->createUrl(['user/bargain/goods/batch']) ?>" data-content="是否批量上架"
						   data-type="0">批量上架</a>
						<a href="javascript:void(0)" class="btn btn-warning batch dropdown-item"
						   data-url="<?= $urlManager->createUrl(['user/bargain/goods/batch']) ?>" data-content="是否批量下架"
						   data-type="1">批量下架</a>
						<a href="javascript:void(0)" class="btn btn-danger batch dropdown-item"
						   data-url="<?= $urlManager->createUrl(['user/bargain/goods/batch']) ?>" data-content="是否批量删除"
						   data-type="2">批量删除</a>
						<a href="javascript:void(0)" class="btn btn-danger batch dropdown-item"
						   data-url="<?= $urlManager->createUrl(['user/bargain/goods/batch']) ?>" data-content="是否批量设置热销"
						   data-type="3">批量设置热销</a>
						<a href="javascript:void(0)" class="btn btn-danger batch dropdown-item"
						   data-url="<?= $urlManager->createUrl(['user/bargain/goods/batch']) ?>" data-content="是否批量取消热销"
				...
				<div class="dropdown float-right ml-2">
					...
					<div class="dropdown-menu" aria-labelledby="dropdownMenuButton"
						...
						<a class="dropdown-item"
						   href="<?= $urlManager->createUrl(['user/bargain/goods/index']) ?>">全部类型</a>
						<?php foreach ($cat_list as $index => $value) : ?>
							<a class="dropdown-item"
							   href="<?= $urlManager->createUrl(array_merge(['user/bargain/goods/index'], $_GET, ['cat' => $value['id']])) ?>"><?= $value['name'] ?></a>
						<?php endforeach; ?>
					</div>
				</div>
			</div>
...
<!--					</td>-->
					...
					<td class="nowrap">
						<a class="btn btn-sm btn-primary"
						   href="<?= $urlManager->createUrl(['user/bargain/goods/goods-edit', 'id' => $goods['id']]) ?>">修改</a>
						<a class="btn btn-sm btn-danger del"
						   href="<?= $urlManager->createUrl(['user/bargain/goods/goods-del', 'id' => $goods['id']]) ?>">删除</a>
                    </td>
				</tr>
			<?php endforeach; ?>
			</tbody>
...
<script>
	...
	function upDown(id, type) {
		...
		var url = "<?= $urlManager->createUrl(['user/bargain/goods/goods-up-down']) ?>";
