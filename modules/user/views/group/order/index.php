                                ...
                                <div class="dropdown float-right ml-2">
                                    ...
                                    <div class="dropdown-menu" style="min-width:8rem">
                                        <a class="dropdown-item"
                                           href="<?= $urlManager->createUrl(['user/group/order/index']) ?>">所有订单</a>
                                        <a class="dropdown-item"
                                           href="<?= $urlManager->createUrl(['user/group/order/index', 'offline' => 1]) ?>">非自提</a>
                                        <a class="dropdown-item"
                                           href="<?= $urlManager->createUrl(['user/group/order/index', 'offline' => 2]) ?>">自提</a>
                                    </div>
                                </div>
            ...
            <ul class="nav nav-tabs status">
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == -1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['user/group/order/index'])) ?>">全部</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 0 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['user/group/order/index'], $condition, ['status' => 0])) ?>">未付款<?= $store_data['status_count']['status_0'] ? '(' . $store_data['status_count']['status_0'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['user/group/order/index'], $condition, ['status' => 1])) ?>">待发货<?= $store_data['status_count']['status_1'] ? '(' . $store_data['status_count']['status_1'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 2 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['user/group/order/index'], $condition, ['status' => 2])) ?>">已发货<?= $store_data['status_count']['status_2'] ? '(' . $store_data['status_count']['status_2'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 6 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['user/group/order/index'], $condition, ['status' => 6])) ?>">拼团中<?= $store_data['status_count']['status_6'] ? '(' . $store_data['status_count']['status_6'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 3 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['user/group/order/index'], $condition, ['status' => 3])) ?>">已完成<?= $store_data['status_count']['status_3'] ? '(' . $store_data['status_count']['status_3'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 5 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['user/group/order/index'], $condition, ['status' => 5])) ?>">已取消<?= $store_data['status_count']['status_5'] ? '(' . $store_data['status_count']['status_5'] . ')' : null ?></a>
                </li>

                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 4 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['user/group/order/index'], $condition, ['status' => 4])) ?>">拼团失败</a>
                </li>
                        ...
                        <td class="order-tab-5">
                                ...
                                <?php if ($order_item['is_cancel'] == 0): ?>
                                    <div class="mb-2">
                                        <a class="btn btn-sm btn-primary apply-status-btn"
                                           href="<?= $urlManager->createUrl(['user/group/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 1]) ?>">同意请求</a>
                                    </div>
                                    <div class="mb-2">
                                        <a class="btn btn-sm btn-danger apply-status-btn"
                                           href="<?= $urlManager->createUrl(['user/group/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 0]) ?>">拒绝请求</a>
                                    </div>
                                <?php endif; ?>
                            <?php endif; ?>
                            ...
                            <?php if ($order_item['is_send'] == 1 && $order_item['pay_type'] == 2 && $order_item['is_confirm'] != 1): ?>
                                ...
                                <a href="javascript:" class="btn btn-sm btn-primary del"
                                   data-url="<?= $urlManager->createUrl(['user/group/order/confirm', 'order_id' => $order_item['id']]) ?>"
...
<script>
    ...
    $(document).on("click", ".send-confirm-btn", function () {
        ...
        console.log(error);
        $.ajax({
            url: "<?=$urlManager->createUrl(['user/group/order/send'])?>",
            type: "post",
...
<script>
    ...
    $(document).on('click', '.print', function () {
        ...
        $.ajax({
            url: "<?=$urlManager->createUrl(['user/group/order/print'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                id: id,
