...
<div class="panel mb-3">
    ...
    <div class="panel-body">
        <div class="float-left pt-2 mb-4">
            <a class="btn btn-primary edit-modal" href="<?= $urlManager->createUrl(['user/store/shop-edit']) ?>">添加门店</a>
        </div>
        <table class="table table-bordered bg-white">
            ...
            <?php foreach ($list as $item) : ?>
                <tr>
                    ...
                    <td>
                        <a href="<?= $urlManager->createUrl(['user/order/index', 'shop_id' => $item['id']]) ?>"><?= $item['order_count'] ?></a>
                    </td>
                    <td><?= $item['total_price'] ?></td>
                    <td>
                        <a href="<?= $urlManager->createUrl(['user/user/card', 'shop_id' => $item['id']]) ?>"><?= $item['card_count'] ?></a>
                    </td>
                    <td>
                    ...
                    <td>
                        <a class="btn btn-primary btn-sm"
                           href="<?= $urlManager->createUrl(['user/store/shop-edit', 'id' => $item['id']]) ?>">修改</a>
                        <a class="btn btn-danger btn-sm del" href="javascript:"
                           data-url="<?= $urlManager->createUrl(['user/store/shop-del', 'id' => $item['id']]) ?>"
...
<script>
    ...
    function upDown(id, type) {
        ...
        var url = "<?= $urlManager->createUrl(['user/store/shop-up-down']) ?>";
        if (confirm("是否" + text + "？")) {
            $.ajax({
                url: url,
                type: 'get',
