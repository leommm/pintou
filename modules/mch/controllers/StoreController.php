...
class StoreController extends Controller
{
    ...
    public function actionShows()
    {
        if (\Yii::$app->request->isPost) {
            ...
            $form->id = $data['id'];
            $form->order_count = $data['order_count'];
            $form->is_show = $data['is_show'];
            $form->notice = $data['notice'];
//            echo "<pre>";print_r(\Yii::$app->request->post());exit;
            return $form->setScreen();
        } else {
            $form = new StoreDataForm();
            $screen = $form->getScreenData();
            $mch=$form->getMchData();
            $cat=$form->getCatData();
    ...
    public function actionShows1()
    {
        if (\Yii::$app->request->isPost) {
            $form = new StoreDataForm();
            $data = \Yii::$app->request->post();
            $form->id = $data['id'];
            $form->deal_money = $data['deal_money'];
            return $form->setScreen1();
        } else {
            $form = new StoreDataForm();
            $screen = $form->getScreenData();
            $mch=$form->getMchData();
            $cat=$form->getCatData();
            return $this->render('shows1', [
                'screen' => $screen,
                'mch'=>$mch,
                'cat'=>$cat
            ]);
    ...
    public function actionShows2()
    {
        if (\Yii::$app->request->isPost) {
            $form = new StoreDataForm();
            $data = \Yii::$app->request->post();
            $form->id = $data['id'];
            $form->cat_price = $data['cat_price'];
//            echo "<pre>";print_r(\Yii::$app->request->post());exit;
            return $form->setScreen2();
        } else {
            $form = new StoreDataForm();
            $screen = $form->getScreenData();
            $mch=$form->getMchData();
            $cat=$form->getCatData();
            return $this->render('shows2', [
                'screen' => $screen,
                'mch'=>$mch,
                'cat'=>$cat
            ]);
    ...
    public function actionShows3()
    {
        if (\Yii::$app->request->isPost) {
            $form = new StoreDataForm();
            $data = \Yii::$app->request->post();
            $form->id = $data['id'];
            $form->people_nums = $data['people_nums'];

//            echo "<pre>";print_r(\Yii::$app->request->post());exit;
            return $form->setScreen3();
        } else {
            $form = new StoreDataForm();
            $screen = $form->getScreenData();
            $mch=$form->getMchData();
            $station=$form->getStationData();
            return $this->render('shows3', [
                'screen' => $screen,
                'mch'=>$mch,
                'station'=>$station
            ]);
...
//            echo "<pre>";print_r(\Yii::$app->request->post());exit;
            return $form->setScreen4();
        } else {
            $form = new StoreDataForm();
            $volume = $form->getVolumeData();
            return $this->render('shows4', [
                'volume' => $volume,
            ]);
        }
    ...
    public function actionVolume($id){
        $volume=Volume::findOne(['id'=>$id]);
        return ['date'=>$volume->total_price];

    }


//获取商户交易额
    public function actionMchmoney($id){
        $money=Mch::findOne(['id'=>$id]);
        return ['date'=>$money->deal_money];

    }

//获取分类交易额
    public function actionCatmoney($id){
        $money=Cat::findOne(['id'=>$id]);

        return ['date'=>$money->cat_price];

    }
    //获取站点人数
    public function actionStationmoney($id){
        $station=Station::findOne(['id'=>$id]);

        return ['date'=>$station->people_nums];
...
//            echo  "<pre>";print_r( $option);exit;
    ...
    {
        $cat_list = Cat::find()->where(['store_id' => $this->store->id, 'is_delete' => 0])->orderBy('sort,addtime DESC')->all();
        $childrenList=$cat_list->childrenList;
        is_null($childrenList)?$childrenList=[]:$childrenList;


        return $this->render('cat', [
            'cat_list' => $cat_list,'childrenList'=>$childrenList
        ]);
    }
