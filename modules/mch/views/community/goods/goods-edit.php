...
if (!$returnUrl) {
    $returnUrl = $urlManager->createUrl(['mch/community/goods/index']);
}
?>
        ...
        <form class="form auto-form" method="post" autocomplete="off" data-return="<?= $returnUrl ?>">
                            ...
                            <div class="input-group short-row">
                                <select class="form-control parent" name="model[cat_id]">
                                    ...
                                    <?php foreach ($cat as $value) : ?>
                                        <option
                                            value="<?= $value['id'] ?>" disabled><?= $value['name'] ?></option>
                                        <?php foreach ($value['child'] as $chi){?>
                                            <option
                                                    value="<?= $chi['id'] ?>" <?= $chi['id'] == $goods['cat_id'] ? 'selected' : '' ?>>&nbsp;&nbsp;<?= $chi['name'] ?></option>
                                        <?php }?>
                                    <?php endforeach; ?>
                                </select>
                            </div>
            ...
            <div class="step-block" flex="dir:left box:first">
                ...
                <div>
                    ...
                    <div class="form-group row">
                        <div class="col-3 text-right">
                            <label class=" col-form-label required">预约金</label>
                        </div>
                        <div class="col-9">
                            <div class="input-group short-row">
                                <input type="number" step="0.01" class="form-control"
            ...
            <div class="step-block" flex="dir:left box:first" style="display: none">
                <div>
                    <span>分销设置</span>
...
//        var error = $('.copy-error');
    ...
    $(document).on('click', '.mall-copy-btn', function () {
        ...
        $.myLoading();
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/community/goods/copy'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                mall_id: mall_id,
