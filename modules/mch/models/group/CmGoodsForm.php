 ...
 */
class CmGoodsForm extends MchModel
{
    public $goods;
    public $goods_share;
    ...
    public function getList($store_id, $mch_id = 0)
    {
        $query = CmGoods::find()
            ...
            ->leftJoin('{{%cm_cat}} c', 'g.cat_id=c.id');
        ...
        foreach ($list as $key => $goods) {
            $list[$key]['num'] = CmGoods::getNum($goods['id']);
        }
        return [$list, $p];
    }
...
//            $this->integral = json_encode($this->integral,JSON_UNESCAPED_UNICODE);
            ...
            if ($goods->save()) {
                CmGoodsPic::updateAll(['is_delete' => 1], ['goods_id' => $goods->id]);
                foreach ($this->goods_pic_list as $pic_url) {
                    $goods_pic = new CmGoodsPic();
