...
use app\models\BrGoodsDetail;

class BrOrderForm extends MchModel
{
    public $offline;
    public $store_id;
    public $user_id;
...
//        $query1 = clone $query;
        $query->innerJoin(['od' => BrOrderDetail::tableName()], 'od.order_id=o.id')
            ->leftJoin(['g' => BrGoods::tableName()], 'g.id=od.goods_id');
		if ($this->flag == "EXPORT") {
    ...
    public function getGroupList()
    {
        ...
        */
        ...
        if ($this->status == 0) {//砍价中
            $query->andWhere([
                'o.status' => 2,
            ]);
        }
        if ($this->status == 1) {//砍价成功
            $query->andWhere([
                'o.status' => 3,
                'o.is_success' => 1,
            ]);
        }
        if ($this->status == 2) {//砍价失败
            $query->andWhere([
                'o.status' => 4,
            ]);
        }
        ...
        foreach ($list as $key => $item) {
            if ($item['class_group']) {
                $detail = BrGoodsDetail::findone(['id' => $item['class_group'], 'store_id' => $this->store_id]);
                $list[$key]['group_num'] = $detail->group_num;
                $list[$key]['colonel'] = $detail->colonel;
            }
            $list[$key]['currentNum'] = BrOrder::find()
    ...
    public function getGroupInfo($pid = 0)
    {
        $query = BrOrder::find()
            ...
            ->leftJoin(['od' => BrOrderDetail::tableName()], 'od.order_id=o.id')
            ->leftJoin(['g' => BrGoods::tableName()], 'g.id=od.goods_id')
            ->leftJoin(['u'=>User::tableName()], 'u.id=o.user_id');
        ...
        foreach ($list as $key => $value) {
            if ($value['class_group']) {
                $detail = BrGoodsDetail::findone(['id' => $value['class_group'], 'store_id' => $this->store_id]);
                $list[$key]['group_num'] = $detail->group_num;
                $list[$key]['colonel'] = $detail->colonel;
            }
    ...
    {
        $order_detail_list = BrOrderDetail::find()->alias('od')
            ->leftJoin(['g' => BrGoods::tableName()], 'od.goods_id=g.id')
            ->where([
                'od.is_delete' => 0,
