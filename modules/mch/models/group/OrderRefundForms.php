...
use app\models\CmNoticeSender;
use app\models\CmOrder;
use app\models\CmOrderDetail;
use app\models\CmOrderRefund;
use app\models\User;
 ...
 * 售后订单结果处理
 */
class OrderRefundForms extends MchModel
{
    public $store_id;
    ...
    public function save()
    {
        ...
        $order_refund = CmOrderRefund::findOne([
    ...
    {
        $order = CmOrder::findOne($order_refund->order_id);
        ...
        if ($this->action == 1) {//同意
            ...
            //恢复库存
            $order_detail = CmOrderDetail::findOne(['order_id' => $order->id, 'is_delete' => 0]);
