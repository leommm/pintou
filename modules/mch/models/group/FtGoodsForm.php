 ...
 */
class FtGoodsForm extends MchModel
{
    public $goods;
    public $goods_share;
    ...
    public function rules()
    {
        return [
            ...
            [['store_id', 'cat_id', 'freight', 'limit_time', 'is_only', 'is_more', 'type', 'use_attr', 'share_type', 'mch_id'], 'integer'],
    ...
    public function getList($store_id, $mch_id = 0)
    {
        $query = FtGoods::find()
            ...
            ->leftJoin('{{%ft_cat}} c', 'g.cat_id=c.id');
        ...
        foreach ($list as $key => $goods) {
            $list[$key]['num'] = FtGoods::getNum($goods['id']);
        }
        return [$list, $p];
    }
...
//            $this->integral = json_encode($this->integral,JSON_UNESCAPED_UNICODE);
            ...
            if ($goods->save()) {
                FtGoodsPic::updateAll(['is_delete' => 1], ['goods_id' => $goods->id]);
                foreach ($this->goods_pic_list as $pic_url) {
                    $goods_pic = new FtGoodsPic();
