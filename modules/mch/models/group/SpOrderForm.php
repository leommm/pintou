...
class SpOrderForm extends MchModel
{
    public $offline;
    public $store_id;
    public $user_id;
...
//        $query1 = clone $query;
        $query->innerJoin(['od' => CmOrderDetail::tableName()], 'od.order_id=o.id')
    ...
    public function getGroupList()
    {
        ...
        $query = CmOrderDetail::find()
            ...
            ->andWhere(['or', ['o.is_pay' => 1], ['o.pay_type' => 2]])
            ->leftJoin(['o' => CmOrder::tableName()], 'od.order_id=o.id')
            ->leftJoin(['g' => CmGoods::tableName()], 'g.id=od.goods_id')
            ->leftJoin(['u' => User::tableName()], 'u.id=o.user_id');
        /*
        $query = PtOrder::find()
        ...
        */
        ...
        foreach ($list as $key => $item) {
            if ($item['class_group']) {
                $detail = CmGoodsDetail::findone(['id' => $item['class_group'], 'store_id' => $this->store_id]);
                $list[$key]['group_num'] = $detail->group_num;
                $list[$key]['colonel'] = $detail->colonel;
            }
            $list[$key]['currentNum'] = CmOrder::find()
    ...
    public function getGroupInfo($pid = 0)
    {
        ...
        foreach ($list as $key => $value) {
            if ($value['class_group']) {
                $detail = CmGoodsDetail::findone(['id' => $value['class_group'], 'store_id' => $this->store_id]);
                $list[$key]['group_num'] = $detail->group_num;
                $list[$key]['colonel'] = $detail->colonel;
            }
    ...
    {
        $order_detail_list = CmOrderDetail::find()->alias('od')
            ->leftJoin(['g' => CmGoods::tableName()], 'od.goods_id=g.id')
            ->where([
                'od.is_delete' => 0,
