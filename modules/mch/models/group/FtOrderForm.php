...
use app\models\FtOrder;
use app\models\FtOrderDetail;
...
class FtOrderForm extends MchModel
{
    public $offline;
    public $store_id;
    public $user_id;
     ...
     * 拼团订单列表
     */
    public function getList($mch_id = 0)
    {
        if (!$this->validate()) {
...
//                'u.nickname',
//            ])
//            ->innerJoin(['o' => PtOrder::tableName()], 'od.order_id=o.id')
...
//            ->andWhere(['o.is_delete' => 0, 'o.store_id' => $this->store_id]);
        ...
        if ($mch_id < 0)
		{
        	...
        	$query->andWhere(['od.mch_id' => $mch_id]);
		}
...
//        $query1 = clone $query;
        $query->innerJoin(['od' => FtOrderDetail::tableName()], 'od.order_id=o.id')
    ...
    public function getGroupList()
    {
        ...
        $query = FtOrderDetail::find()
            ...
            ->andWhere(['or', ['o.is_pay' => 1], ['o.pay_type' => 2]])
            ->leftJoin(['o' => FtOrder::tableName()], 'od.order_id=o.id')
            ->leftJoin(['g' => FtGoods::tableName()], 'g.id=od.goods_id')
            ->leftJoin(['u' => User::tableName()], 'u.id=o.user_id');
        /*
        $query = PtOrder::find()
        ...
        */
        ...
        foreach ($list as $key => $item) {
            if ($item['class_group']) {
                $detail = FtGoodsDetail::findone(['id' => $item['class_group'], 'store_id' => $this->store_id]);
                $list[$key]['group_num'] = $detail->group_num;
                $list[$key]['colonel'] = $detail->colonel;
            }
            $list[$key]['currentNum'] = FtOrder::find()
    ...
    public function getGroupInfo($pid = 0)
    {
        $query = FtOrder::find()
            ...
            ->leftJoin(['od' => FtOrderDetail::tableName()], 'od.order_id=o.id')
            ->leftJoin(['g' => FtGoods::tableName()], 'g.id=od.goods_id')
            ->leftJoin(['u'=>User::tableName()], 'u.id=o.user_id');
        ...
        foreach ($list as $key => $value) {
            if ($value['class_group']) {
                $detail = FtGoodsDetail::findone(['id' => $value['class_group'], 'store_id' => $this->store_id]);
                $list[$key]['group_num'] = $detail->group_num;
                $list[$key]['colonel'] = $detail->colonel;
            }
    ...
    {
        $order_detail_list = FtOrderDetail::find()->alias('od')
            ->leftJoin(['g' => FtGoods::tableName()], 'od.goods_id=g.id')
            ->where([
                'od.is_delete' => 0,
