...
class UserController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $mch=mch::findOne(['user_id'=>\Yii::$app->user->identity->id]);
        $level = $user->level;

        $sc = Goods::find()->from(Favorite::tableName())
            ->alias('f')->leftJoin(['g' => Goods::tableName()], 'f.goods_id=g.id')
            ->where(['f.user_id' => \Yii::$app->user->identity->id, 'f.is_delete' => 0, 'g.is_delete' => 0, 'g.status' => 1]);
        $sccount = $sc->count();
        
        $gz=AttentionStore::find()->alias('a')
          ->leftJoin(['c' => Shop::tableName()], "a.shop_id=c.id")
          ->where(['a.user_id' =>\Yii::$app->user->identity->id]);
          $gzcount=$gz->count();
        
        $ls=GoodsVisitLog::find()->alias('a')
...
//            'parent' => $share ? $share->name : ($parent ? $parent->nickname : '总店'),
    ...
    public function actionUserHandBinding()
    {
        ...
        return new BaseApiResponse($form->send_text($code, $form->attributes['content']));
    }
    ...
    public function actionMember()
    {
        ...
        $percent = 100;
        $s_money = 0;
        if ($next_level && $order_money && $next_level['money'] != 0) {
    ...
    public function actionPaypassword(){
       ...
       $form=new UserForm();
       $form->user_id=\Yii::$app->user->id;
       $form->pay_password=\Yii::$app->request->get('pay_password');
       $form->pay_password1=\Yii::$app->request->get('pay_password1');
       $form->pay_type=\Yii::$app->request->get('pay_type');
       return new BaseApiResponse($form->paypassword());
