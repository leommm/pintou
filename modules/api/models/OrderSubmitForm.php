...
class OrderSubmitForm extends OrderData
{
    ...
    public function save()
    {
        ...
        if (!empty($goods_list)) {//平台自营下的单子
            ...
            // 此处计算所有的优惠措施
            if($user->general_id !=0){
                $order->general_price=$setting->general*0.01 *$total_price_2;
            }else{
                $order->general_price = 0;
            }
            ...
            $order->general_id = $user->general_id;
            $order->first_price = 0;
            $order->second_price = 0;
            $order->third_price = 0;
        ...
        if (!empty($data['mch_list'])) {//入驻商商品下的单子
            foreach ($data['mch_list'] as $mch) {
                ...
                if(is_null($name))$name=$this->address_name;
                if(is_null($mobile))$mobile=$this->address_mobile;
                $order = new Order();
                $order->store_id = $this->store_id;
                $order->user_id = $this->user_id;
                $order->order_no = $this->getOrderNo();
    ...
    public function submitpassword(){
        if(empty($this->password)) return ['code' => 1, 'msg' => '请输入支付密码'];
        $user=User::findOne(['id' =>$this->user_id]);
        if(is_null($user->pay_password)) return ['code' => 1, 'msg' => '尚未设置支付密码'];
        if($user->password = md5($this->password)){
            return ['code' => 0, 'msg' => '密码正确'];
        }else{
