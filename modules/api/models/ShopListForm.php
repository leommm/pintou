<?php
/**
 * Created by PhpStorm.
 * User: Administrator
 * Date: 2017/11/24
 * Time: 10:37
 */

namespace app\modules\api\models;

use app\hejiang\ApiResponse;
use app\models\Mch;
use app\models\Shop;
use app\models\AttentionStore;
use yii\data\Pagination;
use app\models\District;
use app\models\MchVisitLog;
use app\modules\api\models\LocationForm;

class ShopListForm extends ApiModel
{
    public $store_id;
    public $user;
    public $user_id;
    public $longitude;
    public $latitude;
    public $page;
    public $limit;
    public $keyword;
    public $shop_id;
    public $type;
    public function rules()
    {
        return [
            [['longitude', 'latitude','keyword'], 'trim'],
            [['page'], 'integer'],
            [['page'], 'default', 'value' => 0],
            [['limit'], 'integer'],
            [['limit'], 'default', 'value' => 20],
        ];
    }

    public function search()
    {
        if (!$this->validate()) {
            return $this->errorResponse;
        }
        $query = Shop::find()->where([
            'store_id' => $this->store_id,
            'is_delete' => 0
        ]);
        if ($this->keyword) {
            $query->andWhere(['like','name',$this->keyword]);
        }

        $count = $query->count();

        $p = new Pagination(['totalCount' => $count, 'pageSize' => $this->limit, 'page' => $this->page]);

        $list = $query->asArray()->all();
        $distance = array();
        foreach ($list as $index => $item) {
             $attention=AttentionStore::findOne(['shop_id' => $item['id']],['user_id'=>$this->user_id]);
        

             is_null($attention)?$item['attention']=0:$item['attention']=$attention->user_id;
          
                $list[$index]=$item;
                $list[$index]['distance'] = -1;
               
            if ($item['longitude'] && $this->longitude) {
                $from = [$this->longitude, $this->latitude];
                $to = [$item['longitude'], $item['latitude']];
                $list[$index]['distance'] = $this->get_distance($from, $to, false, 2);

            }
            $distance[] = $list[$index]['distance'];
          
        }
        array_multisort($distance, SORT_ASC, $list);
        $min = min(count($list), $this->limit);
        $offset = $this->page * $this->limit;
        $list_arr = array();
        $data_count = 1;
        foreach ($list as $index => $item) {
            if ($index < $offset) {
                continue;
            }
            if ($data_count <= $this->limit) {
                $list[$index]['distance'] = $this->distance($item['distance']);
                array_push($list_arr, $list[$index]);
                $data_count++;
            } else {
                break;
            }
        }
        $data = [
            'list' => $list_arr,
            'page_count' => $p->pageCount,
            'row_count' => $count
        ];
        return new ApiResponse(0, 'success', $data);
    }


    private static function distance($distance)
    {
        if ($distance == -1) {
            return -1;
        }
        if ($distance > 1000) {
            $distance = round($distance / 1000, 2) . 'km';
        } else {
            $distance .= 'm';
        }
        return $distance;
    }

    /**
     * 根据起点坐标和终点坐标测距离
     * @param  [array]   $from  [起点坐标(经纬度),例如:array(118.012951,36.810024)]
     * @param  [array]   $to    [终点坐标(经纬度)]
     * @param  [bool]    $km        是否以公里为单位 false:米 true:公里(千米)
     * @param  [int]     $decimal   精度 保留小数位数
     * @return [string]  距离数值
     */
    function get_distance($from, $to, $km = true, $decimal = 2)
    {
        sort($from);
        sort($to);
        $EARTH_RADIUS = 6370.996; // 地球半径系数

        $distance = $EARTH_RADIUS * 2 * asin(sqrt(pow(sin(($from[0] * pi() / 180 - $to[0] * pi() / 180) / 2), 2) + cos($from[0] * pi() / 180) * cos($to[0] * pi() / 180) * pow(sin(($from[1] * pi() / 180 - $to[1] * pi() / 180) / 2), 2))) * 1000;

        if ($km) {
            $distance = $distance / 1000;
        }

        return round($distance, $decimal);
    }

    public  function attention(){

         $query=AttentionStore::find()->alias('a')
          ->leftJoin(['c' => Mch::tableName()], "a.shop_id=c.id")
          ->where("a.user_id = $this->user_id")
         ->select('a.*,c.*')
         ->asArray();
          $count=$query->count();
          $list=$query->all();

//          var_dump($list);exit;
            foreach ($list as $k=>$v){
                $visit_num=MchVisitLog::find()->where(['mch_id'=>$v['id']])->asArray()->count();
                $from=[\Yii::$app->request->get('longitude'),\Yii::$app->request->get('latitude')];
                $to=[$v['longitude'],$v['latitude']];
                $location=new LocationForm();
                $distance="未知";//获取用户和商户的距离km
                if(!empty($from)){
                    if(!empty($v['longitude']) && !empty($v['latitude'])){
                        $distance=$location->get_distance($from,$to);//获取用户和商户的距离km
                    }
                }
                $list[$k]['distance']=$distance;
                $list[$k]['visit_num']=$visit_num;
                $ids=array();
                $ids=[$v['province_id'],$v['city_id'],$v['district_id']];
                $loca_info=District::find()->where(["in","id",$ids])->asArray()->all();
                $mch_address=$v['address'];



                $list[$k]['address']=$mch_address;
            }
          $pagination = new Pagination(['totalCount' => $count, 'page' => $this->page - 1, 'pageSize' => $this->limit]);

       $data = [
            'list' => $list,
            'page_count' => $pagination->pageCount,
            'row_count' => $count
        ];
        return new ApiResponse(0, 'success', $data);

    }


    public function payshop(){

        if (is_null($this->shop_id)) { $data = ['msg' => "缺少店铺id",'code' => 1]; return new ApiResponse(0, 'success', $data);}        
         $attention=AttentionStore::findOne(['shop_id' => $this->shop_id,'user_id'=>$this->user_id]);
//        var_dump($attention);exit;
        if($this->type==1){
            if(!is_null($attention)){ $data = ['msg' => "不能重复关注",'code' => 1]; return new ApiResponse(0, 'success', $data);} 
            if($attention->is_delete == 1){
                 $add = \Yii::$app->db->createCommand()->update('cshopmall_attention_store', ['is_delete'=>0], "shop_id = {$this->shop_id} and user_id = {$this->user_id}")->execute();
            }else{
                 $add= \Yii::$app->db->createCommand('INSERT INTO `cshopmall_attention_store` (`user_id`,`shop_id`,`is_delete`,`addtime`) VALUES (:user_id,:shop_id,:is_delete,:addtime)', [
                ':user_id' =>$this->user_id,
                ':shop_id'=>$this->shop_id,
                ':is_delete'=>0,
                ':addtime'=>time(),

            ]  )->execute();
            }       
           
        }elseif ($this->type==2) {
            $add = \Yii::$app->db->createCommand()->delete('cshopmall_attention_store', "user_id = {$this->user_id} and shop_id  = {$this->shop_id}")->execute();
            
            

        }
        
        
        if ($add>0) {
          $data = [
            'msg' => "成功",
            'code' => 0,
        ];
    
        } 
        return new ApiResponse(0, 'success', $data);

    }

}
