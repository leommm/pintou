...
class TopicForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        $model = Topic::find()->where([ 'id' => $this->id, 'is_delete' => 0])
            ->select('id,title,sub_title,cover_pic,read_count,virtual_read_count,content,addtime')->asArray()->one();
        ...
        if ($model['read_count'] >= 10000) {
            $model['read_count'] = round($model['read_count'] / 10000,2) . 'W次浏览';
        }
