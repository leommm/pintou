...
class OrderSubmitPreviewFormBr extends ApiModel
{
	public $store_id;
	public $user_id;
...
//        $res['data']['level'] = $level;
	...
	private function getDataByGoodsInfo($goods_info, $store)
	{
		...
		$goods = $table->createCommand("select * from {{%br_goods}} where id = ".
			$goods_info->goods_id." and is_delete = 0 and store_id = ".$this->store_id." and status = 1 limit 1")->queryOne();
		if (!$goods) {
			return [
				'code' => 1,
				'msg' => '商品不存在或已下架',
		...
		$bargain = $table->createCommand("select * from {{%bargainirg}} where id = ".$this->br_id)->queryOne();
		$total_price = 0;
		$goods_item = (object)[
			'goods_id' => $goods['id'],
			'goods_name' => $goods['name'],
			'num' => $goods_info->num,
			'price' => $bargain['deal_money'] * $goods_info->num,
			'single_price' => $goods['original_price'],
		...
		if ($this->type == 'ONLY_BUY') {
			...
			$orderNum = BrOrder::find()
				...
				->count();
			if (!empty($goods->fav_num) && $goods->fav_num <= $orderNum) {
				return [
					'code'  => 1,
					'msg'   => '您已超过该商品购买次数',
				];
