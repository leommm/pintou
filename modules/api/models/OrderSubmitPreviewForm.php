...
class OrderSubmitPreviewForm extends OrderData
{
    ...
    public function     search()
    {
        ...
        if ($store->send_type != 1) {
            if(!isset($array['mch_id']))$array['mch_id']=0;
            $res['data']['shop_list'] = $this->getShopList($array['mch_id']);
            $res['data']['is_shop'] = Shop::find()->where(['is_default' => 1,'mch_id'=>$array['mch_id'] ,'store_id' => $this->store_id])->asArray()->one();
        } else {
            $res['data']['shop_list'] = [];
            $res['data']['is_shop'] = '';
        }
//        }
    ...
    private function getShopList($mch)
    {


        $list = Shop::find()->select(['address', 'mobile', 'id', 'name', 'longitude', 'latitude'])
            ->where(['store_id' => $this->store_id,'mch_id'=>$mch, 'is_delete' => 0])->asArray()->all();
        $distance = array();
