...
class UserForm extends ApiModel
{
    ...
    public function rules()
    {
        return [
            [['user_id',], 'required'],
            [['binding', 'phone_code', 'pay_password','bind_type'], 'integer'],
    ...
    public function paypassword(){
        if(is_null($this->pay_type))return ['code'=>1, 'msg'=>'缺少参数pay_type'];


        if($this->pay_type == 1){
            $user=User::findOne(['id'=>$this->user_id]);
            if($user->pay_password == md5($this->pay_password)) {
                return ['code'=>1, 'msg'=>'不能与之前密码一致'];

            }
            if(is_null($this->pay_password))return ['code'=>1, 'msg'=>'支付密码不能为空'];
            if(strlen($this->pay_password)<6)return ['code'=>1, 'msg'=>'支付密码不能小于6位数'];
            $up = \Yii::$app->db->createCommand()->update('cshopmall_user', ['pay_password' => md5($this->pay_password)], "id = {$this->user_id}")->execute();
        }elseif($this->pay_type == 2){


            $user=User::findOne(['id'=>$this->user_id]);
            if(is_null($user->pay_password)){
                return ['code'=>2, 'msg'=>'尚未设置支付密码'];
            }
            if($user->pay_password != md5($this->pay_password1)) {
                return ['code'=>1, 'msg'=>'原先支付密码错误'];

            }else{
                return ['code'=>0, 'msg'=>'支付密码正确'];
