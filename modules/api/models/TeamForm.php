...
class TeamForm extends ApiModel
{
    ...
    public function getTeam()
    {
      ...
      /*  if(!is_null($this->time))echo 1;
            if(!is_null($this->time1))echo 2;
                if(!is_null($this->time) AND !is_null($this->time1))echo 3;
        die;*/


      /*  if(!is_null($this->time))$sql .= " WHERE {$this->time} <= u.time  ";
        if(!is_null($this->time1))$sql .= " WHERE u.time < {$this->time1} ";
        if(!is_null($this->time) AND !is_null($this->time1))$sql .= " WHERE  {$this->time} <= u.time AND u.time < {$this->time1} ";*/
        ...
        if ($this->status == 1) {
            ...
            if(!is_null($this->time) && !is_null($this->time1)){
                $query.=" AND {$this->time} <= u.time AND u.time < {$this->time1} ";
            }
        } elseif ($this->status == 2) {
            if (empty($team_arr['s_c'])) {
            ...
            $query .= " LEFT JOIN {$sql} as al ON al.user_id =u.id
                WHERE u.store_id = {$this->store_id}  AND u.id IN {$second} AND {$this->time} <= u.time AND u.time < {$this->time1}";
        } else {
        ...
        foreach ($list as $index => $value) {
            ...
            $list[$index]['phone']=substr_replace($user->binding,'****',3,4);
            $time=(time()-$value['addtime'])/3600;

            $list[$index]['time']=round($time,2);
        ...
        if(!is_null($list)){
            $arr_price = $this->i_array_column($list,'price');
         $data['commission'] = array_sum($arr_price);}
